...
/// [MS-WMF] 2.3.6.27 NEWFRAME Record
...
public struct NEWFRAME {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// EscapeFunction (2 bytes): A 16-bit unsigned integer that defines the escape function. The value MUST be 0x0001
        /// (NEWFRAME) from the MetafileEscapes Enumeration (section 2.1.1.17) table.
        self.escapeFunction = try MetafileEscapes(dataStream: &dataStream)
        guard self.escapeFunction == .NEWFRAME else {
            throw MetafileReadError.corrupted
        }
