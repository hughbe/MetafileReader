...
/// [MS-WMF] 2.3.6.32 POSTSCRIPT_IGNORE Record
...
public struct POSTSCRIPT_IGNORE {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// EscapeFunction (2 bytes): A 16-bit unsigned integer that defines the escape function. The value MUST be 0x0026
        /// (POSTSCRIPT_IGNORE) from the MetafileEscapes Enumeration (section 2.1.1.17) table.
        self.escapeFunction = try MetafileEscapes(dataStream: &dataStream)
        guard self.escapeFunction == .POSTSCRIPT_IGNORE else {
            throw WmfReadError.corrupted
        }
