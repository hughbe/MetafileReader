...
/// [MS-WMF] 2.3.6.26 METAFILE_DRIVER Record
...
public struct METAFILE_DRIVER {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// EscapeFunction (2 bytes): A 16-bit unsigned integer that defines the escape function. The value MUST be 0x0801
        /// (METAFILE_DRIVER) from the MetafileEscapes Enumeration (section 2.1.1.17) table.
        self.escapeFunction = try MetafileEscapes(dataStream: &dataStream)
        guard self.escapeFunction == .METAFILE_DRIVER else {
            throw WmfReadError.corrupted
        }
