...
/// [MS-WMF] 2.3.6.15 EXTTEXTOUT Record
...
public struct EXTTEXTOUT {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// EscapeFunction (2 bytes): A 16-bit unsigned integer that defines the escape function. The value MUST be 0x0200
        /// (EXTTEXTOUT) from the MetafileEscapes Enumeration (section 2.1.1.17) table.
        self.escapeFunction = try MetafileEscapes(dataStream: &dataStream)
        guard self.escapeFunction == .EXTTEXTOUT else {
            throw MetafileReadError.corrupted
        }
