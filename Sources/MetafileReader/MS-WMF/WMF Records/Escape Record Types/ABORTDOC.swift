...
/// [MS-WMF] 2.3.6.2 ABORTDOC Record
/// The ABORTDOC Record stops processing the current document and erases everything drawn since the last STARTDOC Record
/// (section 2.3.6.44) was processed.
/// See section 2.3.6 for the specification of other Escape Record Types.
public struct ABORTDOC {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        /// EscapeFunction (2 bytes): A 16-bit unsigned integer that defines the escape function. The value MUST be 0x0002
        /// (ABORTDOC) from the MetafileEscapes Enumeration (section 2.1.1.17) table.
        self.escapeFunction = try MetafileEscapes(dataStream: &dataStream)
        guard self.escapeFunction == .ABORTDOC else {
            throw MetafileReadError.corrupted
        }
