...
public struct DeviceIndependentBitmap {
    ...
    public init(dataStream: inout DataStream) throws {
        ...
        let colorUsed: Int?
        if case .bitmapInfoHeader(let bitmapInfoHeader) = self.dibHeaderInfo, bitmapInfoHeader.colorUsed != 0 {
            colorUsed = Int(bitmapInfoHeader.colorUsed)
        } else {
            colorUsed = nil
        }
    ...
    public enum Header {
        ...
        var headerSize: UInt32 {
            switch self {
            case .coreHeader(let coreHeader):
                return coreHeader.headerSize
            case .bitmapInfoHeader(let bitmapInfoHeader):
                return bitmapInfoHeader.headerSize
            }
        }
    }
